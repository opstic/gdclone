...
pub(crate) struct UfoMode;

const GRAVITY: f64 = 0.958199;

impl PlayerFunction for UfoMode {
    fn update(
        &mut self,
        world: &mut World,
        _: Entity,
        ...
        let system_state: &mut SystemState<UfoSystemParam> =
        ...
        if mouse_input.just_pressed(MouseButton::Left) {
            let base_velocity = if !player.mini { 7. } else { 8. };
            player.velocity.y = base_velocity * mini_factor;
        }

        let gravity_factor = if !player.falling() { 1.2 } else { 0.8 };

        player.velocity.y -=
            GRAVITY * (60. * 0.9 * time.delta_seconds_f64()) * gravity_factor * 0.5 / mini_factor;
    ...
    fn create_system_state(&self, world: &mut World) -> Box<dyn Any + Send + Sync> {
        Box::new(SystemState::<UfoSystemParam>::new(world))
    }
}
